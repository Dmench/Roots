...
type View = 'signin' | 'signup' | 'confirm-email' | 'reset-sent'
...
export function AuthModal({ isOpen, onClose, returnTo }: Props) {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    setLoading(false)
    if (err) setError(err.message.includes('Invalid login') ? 'Wrong email or password.' : err.message)
  ...
  const isPostSubmit = view === 'confirm-email' || view === 'reset-sent'
  const postHeading  = view === 'confirm-email' ? 'Check your email' : 'Password reset sent'
  const postBody     = view === 'confirm-email'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-cream border border-sand/60 rounded-none p-8 w-full max-w-sm">
        ...
        {isPostSubmit && (
          <div className="text-center py-2">
            ...
            <h3 className="font-display font-bold text-espresso text-xl mb-3">{postHeading}</h3>
            <p className="text-walnut/60 text-sm leading-relaxed">{postBody}</p>
            {view === 'confirm-email' && (
              <p className="text-xs text-walnut/40 mt-3">
                Didn&apos;t get it? Check spam, or{' '}
                <button onClick={() => { setView('signup'); setPassword(''); setConfirm('') }}
        ...
        {!isPostSubmit && (
          <>
            <div className="flex gap-1 p-1 rounded-none mb-6" style={{ background: 'rgba(37,36,80,0.06)' }}>
              {(['signin', 'signup'] as View[]).map(v => (
                <button key={v}
                  ...
                  style={view === v ? { background: '#252450', color: '#F5F4F0' } : { color: 'rgba(37,36,80,0.45)' }}>
            ...
            <form onSubmit={view === 'signin' ? handleSignIn : handleSignUp} className="space-y-3">
              ...
              {error && <p className="text-xs leading-snug" style={{ color: '#C8152A' }}>{error}</p>}
