...
export default function EventsSection({
  ...
  return (
    <section>
      ...
      {/* ── Venue filter — underline tabs ───────────────────────────────── */}
      <div className="flex items-center gap-5 flex-wrap mb-8"
        style={{ borderBottom: '1px solid rgba(10,10,10,0.08)', paddingBottom: 12 }}>
        <button
          onClick={() => setActiveSource(null)}
          className="text-[10px] font-black tracking-[0.15em] uppercase pb-0.5 transition-all"
          style={!activeSource
            ? { color: '#0A0A0A', borderBottom: '2px solid #0A0A0A' }
            : { color: 'rgba(10,10,10,0.3)', borderBottom: '2px solid transparent' }
          }
        >
          All venues
        </button>
      ...
      <div className="space-y-14">
        {buckets.map(bucket => {
          ...
          const accentColor  = isTonight ? '#FF3EBA' : isWeekend ? '#FAB400' : 'rgba(10,10,10,0.15)'

          return (
            <div key={bucket.label}>
              {/* Section header — bulletin-size */}
              <div className="flex items-end justify-between mb-6 pb-3"
                ...
                <div className="flex items-baseline gap-3">
                  <h2 className="font-display font-black leading-none"
                    style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', color: '#0A0A0A', letterSpacing: '-0.02em' }}>
                    {labelText}
                  </h2>
                  <span className="text-xs font-medium" style={{ color: 'rgba(10,10,10,0.3)' }}>
                ...
                {items.map(({ ev, dates }, idx) => {
                  ...
                  return (
                    <a
                      ...
                      className={
                        isDarkHero
                          ? 'group overflow-hidden hover:opacity-90 transition-opacity flex flex-col sm:col-span-2 relative'
                          : isHero
                          ? 'group overflow-hidden hover:opacity-90 transition-opacity flex flex-col sm:col-span-2 relative'
                          : 'group overflow-hidden hover:opacity-80 transition-opacity flex flex-col relative'
                      }
                      style={isDarkHero
                        ? { background: '#0A0A0A', border: '1px solid rgba(255,62,186,0.2)' }
                        : { border: '1px solid rgba(10,10,10,0.1)', background: '#FFFFFF' }}
                    >
                      {/* Save button */}
                      ...
                      {ev.image ? (
                        <div className={`w-full overflow-hidden relative shrink-0 ${isHero ? 'h-52' : 'h-36'}`}>
                          ...
                          {/* Venue label — no rounding */}
                          <span
                            className="absolute top-3 left-3 text-[9px] font-black px-2 py-0.5"
                            style={{ background: venueColor, color: '#fff', letterSpacing: '0.07em' }}>
                            {ev.source.toUpperCase()}
                          </span>
                          {/* Date + title on image for hero */}
                          {isHero && (
                      ...
                      {!isHero && (
                        <div className="px-4 py-4 flex flex-col flex-1">
                          <p className="text-[10px] font-black tracking-wider uppercase mb-2"
                            ...
                            {dates.length > 1 && (
                              <span className="ml-2 opacity-40" style={{ color: '#0A0A0A' }}>
                                +{dates.length - 1}
                              </span>
                            )}
                          </p>
                          ...
                          {ev.venue && ev.venue !== ev.source && (
                            <p className="text-[10px] mt-2 truncate" style={{ color: 'rgba(10,10,10,0.35)' }}>
                              {ev.venue}
                            </p>
                          )}
                        </div>
                      ...
                      {isHero && !ev.image && (
                        <div className="px-5 py-5 flex flex-col flex-1">
                          ...
                          <p className="font-display font-bold leading-snug flex-1"
                            style={{ fontSize: '1.2rem', color: isDarkHero ? '#FFFFFF' : '#0A0A0A' }}>
                            {ev.title}
                          </p>
                          {ev.venue && ev.venue !== ev.source && (
                            <p className="text-[10px] mt-3 truncate"
                              style={{ color: isDarkHero ? 'rgba(255,255,255,0.35)' : 'rgba(10,10,10,0.35)' }}>
                              {ev.venue}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="px-4 py-3 flex items-center justify-between"
                          ...
                          <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                            {dates.slice(1, 4).map((d, i) => (
                              <span key={i} className="text-[10px] font-bold"
                                style={{ color: isDarkHero ? 'rgba(255,255,255,0.35)' : 'rgba(10,10,10,0.4)' }}>
                                {d.date.split(' ').slice(0, 3).join(' ')}
                              </span>
                            ))}
                            {dates.length > 4 && (
                              <span className="text-[10px] opacity-35"
                                style={{ color: isDarkHero ? '#FFFFFF' : '#0A0A0A' }}>
                                +{dates.length - 4} more
                              </span>
                            )}
                          </div>
                          {ev.venue && ev.venue !== ev.source && (
                            <p className="text-[10px] shrink-0 ml-3"
                              style={{ color: isDarkHero ? 'rgba(255,255,255,0.3)' : 'rgba(10,10,10,0.35)' }}>
                              {ev.venue}
                            </p>
                          )}
                        </div>
      ...
      {filtered.length === 0 && (
        <div className="py-12 text-center">
          <p className="text-sm" style={{ color: 'rgba(10,10,10,0.3)' }}>
